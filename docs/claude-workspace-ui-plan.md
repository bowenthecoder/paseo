...
The combined app was pushed at `88446f0b8`; the subscriptions integration was pushed at
`a77d688`. Draft reviews are [app #1](https://github.com/bowenthecoder/paseo/pull/1) and
...
| Subscriptions                  | Registration (15), quota (13), Tool logs (9) and Routines (289) focused checks passed. Types and real plugin bundles passed. The installed-plugin browser flow passes verbose/compact activity, both toggle directions and reload persistence with no page exceptions. Scoped lint is clean; the broader plugin retains eleven baseline findings.                                                                                                                 |
...
| Source publication / CI        | Both integration branches are pushed with draft PRs. Fork Actions registration was enabled. The first full run [34302543655](https://github.com/bowenthecoder/paseo/actions/runs/34302543655) was canceled after identifying obsolete sidebar selectors and two server fixture omissions. Its app tests (5,039), SDK, relay, CLI shards, Windows desktop tests and quality checks passed. Corrected fixtures and migrated browser flows require a fresh full run. |
