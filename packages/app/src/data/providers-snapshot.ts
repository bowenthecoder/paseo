import { CancelledError, isCancelledError, type QueryClient } from "@tanstack/react-query";
...
export async function refreshAndApplyProvidersSnapshot(input: {
  ...
  try {
    await input.queryClient.fetchQuery({
      queryKey,
      staleTime: 0,
      // A refresh-created query keeps its server-data route meta and stays retryable.
      gcTime: Infinity,
