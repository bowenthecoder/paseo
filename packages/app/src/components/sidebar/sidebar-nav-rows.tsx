...
const SidebarNewWorkspaceRow = memo(function SidebarNewWorkspaceRow({
  ...
  const isManualGrouping = useSidebarViewStore((state) => state.groupMode === "manual");
  ...
  const handlePress = useCallback(() => {
    onBeforeNavigate?.();
    // Manual chat groups own their placement, so a new chat starts unscoped
    // instead of inheriting the active workspace's folder.
    if (isManualGrouping) {
  ...
  return (
    <SidebarHeaderRow
      icon={Plus}
      label={isManualGrouping ? "New chat" : t(builtinSidebarNavLabelKey("new-workspace"))}
      onPress={handlePress}
