...
describe("ZoomableViewport toolbar", () => {
  it("reveals controls when image loading replaces content beneath the pointer", async () => {
    const container = document.createElement("div");
    container.style.cssText = "display:flex;width:480px;height:320px;margin:40px;";
    document.body.appendChild(container);
    const root = createRoot(container);
    mounted.push({ root, container });
    act(() => root.render(<div style={LOADING_STYLE}>Loading image</div>));
    await act(async () => userEvent.hover(container));
    act(() =>
      root.render(
        <ZoomableViewport contentSize={CONTENT_SIZE} testID="loaded-preview">
          Loaded image
        </ZoomableViewport>,
      ),
    );

    const canvas = requiredElement(container, '[data-testid="loaded-preview-canvas"]');
    ...
    await expect.poll(() => getComputedStyle(zoomIn).opacity).toBe("1");
