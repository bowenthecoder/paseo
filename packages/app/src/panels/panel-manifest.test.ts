...
describe("panel manifest", () => {
  it("answers host support without React panel registration", () => {
    ...
    expect(panelSupportsHost("subagents", "main")).toBe(false);
    expect(panelSupportsHost("subagents", "explorer")).toBe(true);
    expect(panelSupportsHost("provider_subagent", "main")).toBe(false);
    expect(panelSupportsHost("provider_subagent", "explorer")).toBe(true);
  ...
  it("does not reuse a parent's Changes for a different worktree", () => {
    expect(panelResourceKey({ kind: "working_diff" })).not.toBe(
      panelResourceKey({ kind: "working_diff", workspaceId: "child-worktree" }),
    );
    expect(panelResourceKey({ kind: "working_diff" })).not.toBe(
      panelResourceKey({ kind: "working_diff", workspaceId: "working_diff" }),
    );
  });
});
