 ...
 */
test("a workspace shows one chat, with its terminal in the side panel", async ({ page }) => {
...
test("the sidebar chat menu has no split entries", async ({ page }) => {
  ...
  try {
    ...
    const chatKey = `${getServerId()}:chat:${workspace.agents[0].id}`;
    await page.getByTestId(`sidebar-workspace-row-${chatKey}`).click({ button: "right" });
    await expect(page.getByTestId(`sidebar-workspace-context-menu-${chatKey}`)).toBeVisible();
