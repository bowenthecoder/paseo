...
test.describe("CodeMirror workspace file editing", () => {
  ...
  test("warns before closing a panel with an unsaved draft", async ({ page, withWorkspace }) => {
    ...
    await page.getByTestId("workspace-side-panel-close-view-file_draft.ts").click();
    await expect.poll(() => closePrompt).toContain("Closing it will discard the draft.");
  ...
  test("refreshes Markdown and images while preserving Preview and Source behavior", async ({
    ...
    await expect(zoomInButton).toHaveCSS("opacity", "1");
    await expect(zoomInButton).toHaveCSS("pointer-events", "auto");
    await zoomInButton.click({ timeout: 10_000 });
    await expect
