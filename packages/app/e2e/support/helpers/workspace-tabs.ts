...
export async function openPullRequestPanel(page: Page): Promise<void> {
  ...
  const existingView = visibleTestId(page, "workspace-side-panel-view-pull_request").first();
  if ((await existingView.count()) > 0) {
    await existingView.click();
  } else {
    await openChangesTreePanel(page);
    const pullRequest = visibleTestId(page, "changes-open-pull-request").first();
    await expect(pullRequest).toBeVisible({ timeout: 15_000 });
...
export async function expectSetupTabNotSeeded(page: Page, workspaceId: string): Promise<void> {
  ...
  // A hidden dock does not mount its panel hosts. Reveal it to inspect its actual views.
  await ensureExplorerSidebar(page);
  const view = page.getByTestId(`workspace-side-panel-view-setup_${workspaceId}`);
...
export async function expectFailedSetupSeededInSidePanel(
  ...
  await ensureExplorerSidebar(page);
  const view = visibleTestId(page, `workspace-side-panel-view-setup_${workspaceId}`);
  await expect(view).toHaveCount(1, { timeout: 30_000 });
  await view.click();
...
export async function closeSetupTab(page: Page, workspaceId: string): Promise<void> {
  const testId = workspacePanelTestId("setup", workspaceId);
  const close = visibleTestId(page, `workspace-side-panel-close-view-setup_${workspaceId}`);
  await expect(close).toBeVisible({ timeout: 30_000 });
  await close.click();
  await expect(page.getByTestId(testId)).toHaveCount(0);
  await expect(page.getByTestId(`workspace-side-panel-view-setup_${workspaceId}`)).toHaveCount(0);
}
...
export async function expectNoTerminalTabs(page: Page): Promise<void> {
  await ensureExplorerSidebar(page);
  await expect(page.locator('[data-testid^="workspace-side-panel-view-terminal_"]')).toHaveCount(0);
}
